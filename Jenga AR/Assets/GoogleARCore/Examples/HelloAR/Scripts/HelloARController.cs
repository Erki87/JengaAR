...
namespace GoogleARCore.Examples.HelloAR
{
    ...
    public class HelloARController : MonoBehaviour
    {
        ...
        private System.Collections.IEnumerator Placing()
        {
            int counter = 0;
                ...
                if (Input.touchCount > 0 && (touch = Input.GetTouch(0)).phase == TouchPhase.Began)
                {
                    ...
                    if (Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out hit))
                    {
                        ...
                        else
                        {
                            ...
                            placedTower = Instantiate(AndyAndroidPrefab, hit.Pose.position, hit.Pose.rotation);

                            // Compensate for the hitPose rotation facing away from the raycast (i.e. camera).
                            placedTower.transform.Rotate(0, k_ModelRotation, 0, Space.Self);
        ...
        private void PerformGrabbing()
        {
            ...
            if (Input.touchCount > 0 && (touch = Input.GetTouch(0)).phase == TouchPhase.Began)
            {
                ...
                {
                    if (hit.transform.name.StartsWith("Stone"))
                    {
                        hit.transform.parent = FirstPersonCamera.transform;
